...
- **Extracción (Scraping):** Uso de `BeautifulSoup` y `Requests` con gestión de cabeceras para una extracción eficiente y ética.
- **Paginación Automática:** Capacidad para escalar el scraping a múltiples páginas de resultados de forma iterativa.
- **Transformación de Datos (Cleaning):** Limpieza de strings, eliminación de ruidos (tabulaciones, saltos de línea) y normalización de nombres mediante **Regex** y **Pandas**.
- **Almacenamiento (SQL):** Implementación de persistencia de datos en una base de datos relacional **SQLite** y exportación paralela a formato **CSV**.
- **Visualización de Datos:** Generación de insights mediante un **Jupyter Notebook** interactivo con gráficas de tendencias.
